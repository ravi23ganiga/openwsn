...
#include "svc_nio_datatree_csma.h"
...
TiDataTreeNetwork * dtp_open( TiDataTreeNetwork * net, TiCsma * mac, uint16 localaddress, 
		...
		case DTP_STATE_IDLE:
			...
			*/
			...
			{  
				...
				{
				case DTP_BROADCAST:
					if (csma_broadcast( net->mac, net->txque, 0x00 ) > 0)//if (aloha_broadcast( net->mac, net->txque, 0x00 ) > 0)
					{
						frame_totalclear( net->txque );
						net->txtrytime = DTP_MAX_TX_TRYTIME;
					}
				...
				case DTP_UNICAST_LEAF2ROOT:
					...
					if (csma_send( net->mac,net->parent, net->txque, 0x00 ) > 0)//if (aloha_send( net->mac,net->parent, net->txque, 0x01 ) > 0)
					{   
						frame_totalclear( net->txque );
						net->txtrytime = DTP_MAX_TX_TRYTIME;
					}
		...
		case DTP_STATE_IDLE:
			...
			{   
				...
				case DTP_UNICAST_ROOT2LEAF:
					...
					{
						//todo if (aloha_send( net->mac, net->txque, 0x01 ) > 0)
						if (csma_send( net->mac, DTP_SHORTADDRTO( pkt),net->txque, 0x00 ) > 0)//if (aloha_send( net->mac, DTP_SHORTADDRTO( pkt),net->txque, 0x01 ) > 0)//todo 这一句不确定
						{
							frame_totalclear( net->txque );
							net->txtrytime = DTP_MAX_TX_TRYTIME;
						}
...
bool _dtp_evolve_recv_check( TiDataTreeNetwork * net )
{
	...
	count = csma_recv( net->mac, net->rxbuf, 0x00 );//count = aloha_recv( net->mac, net->rxbuf, 0x00 );
	if (count <= 0)
	{	
		ret = false;
	}
...
void _dtp_evolve_sink_recv( TiDataTreeNetwork * net )
{
	...
	case DTP_MULTICAST:
		...
		//todo _switch_ptr( &(net->rxbuf), &(net->rxque) );
		frame_totalclear( net->rxbuf );
		break;
