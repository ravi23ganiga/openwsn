 ...
 * ------------------------------------------------------------------------------------------------
 */
#define spix_init(spi) macSpiInit()
void macSpiInit(void);
//void spix_init( TSpi * spi );

#define spix_cmdstrobe(spi,regaddr)  macSpiCmdStrobe(regaddr)
uint8 macSpiCmdStrobe(uint8 regAddr);

#define spix_readreg(spi,regaddr) macSpiReadReg(regaddr)
uint16 macSpiReadReg(uint8 regAddr);

#define spix_writereg(spi,regaddr, regvalue) macSpiWriteReg(regaddr, regvalue)
void macSpiWriteReg(uint8 regAddr, uint16 regValue);

#define spix_writeram(spi,ramaddr,data,len) macSpiWriteRam(ramaddr,data,len)
void macSpiWriteRam(uint16 ramAddr, uint8 * pWriteData, uint8 len);

#define spix_readram(spi,ramaddr,data,len) macSpiReadRam(ramaddr,data,len)
void macSpiReadRam(uint16 ramAddr, uint8 * pReadData, uint8 len);

#define spix_write16(spi,ramaddr,data) macSpiWriteRamUint16(ramaddr,data)
void macSpiWriteRamUint16(uint16 ramAddr, uint16 data);

#define spix_writefifo(spi,regaddr,data,len) macSpiWriteFifo(regaddr,data,len)
void macSpiWriteFifo(uint8 regAddr, uint8 * pWriteData, uint8 len);

#define spix_readfifo(spi,regaddr,data,len) macSpiReadFifo(regaddr,data,len)
