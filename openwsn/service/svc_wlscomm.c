...
TiWlsComm * wls_construct( char * buf, uint16 size, TRfDriver * rf, TActionScheduler * actsche )
{
	uint16 len;
	TiWlsComm * wls;

	len = sizeof(TiWlsComm);
	if (len > size)
		wls = NULL;
	else
		wls = (TiWlsComm *)buf;
 ...
 * read received data from TiWlsComm object's internal buffer. these data
 ...
 * this function will write data to "TiWlsComm"'s internal buffer, and then
 ...
 * this function will write the whole frame payload to "TiWlsComm"'s internal
