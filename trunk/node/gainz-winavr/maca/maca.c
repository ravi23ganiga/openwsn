...
int maca_node()
{
    ...
    maca_setchannel( mac, CONFIG_CHANNEL );
	maca_setpanid( mac, CONFIG_PANID );					    /* network identifier */
	maca_setshortaddress( mac, CONFIG_LOCALADDRESS );       /* in network node identifier */
	mac = maca_open( mac, cc2420_interface(cc), cc, timer, NULL, NULL );
