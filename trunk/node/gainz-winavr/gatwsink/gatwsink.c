...
int main(void)
	...
	cc2420_open(cc, 0, NULL, NULL, 0x01 );
	...
	while(1)
	{
		...
		while (!vti_expired(vti))
		{
			...
			{	
				/*pc = &opf->buf[0];
				for(uint8 i=0;i<opf_framelength(opf)+1;i++)
					dbo_n8toa( pc[i] );*/
				...
				{
					iobuf_clear(buf);
					count = opf_framelength(opf)+1;
					iobuf_pushbyte( buf, 0x55 );
					iobuf_pushbyte( buf, count>>8 );
					iobuf_pushbyte( buf, count );
					iobuf_pushback( buf, &opf->buf[0], count );
